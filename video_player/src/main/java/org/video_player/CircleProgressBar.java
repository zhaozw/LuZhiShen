...
public class CircleProgressBar extends android.support.v7.widget.AppCompatImageView {
    ...
    private static final int DEFAULT_CIRCLE_BACKGROUND = 0xFFD03D4B;
    private static final int DEFAULT_CIRCLE_PROGRESS = 0xFFF0B568;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mBackGroundColor = a.getColor(
                R.styleable.CircleProgressBar_background_color, DEFAULT_CIRCLE_BACKGROUND);

        int progressColor = a.getColor(
                R.styleable.CircleProgressBar_progress_color, DEFAULT_CIRCLE_PROGRESS);
        mColors = new int[]{progressColor};
        ...
        mProgress = a.getInt(R.styleable.CircleProgressBar__progress, 0);
        mMax = a.getInt(R.styleable.CircleProgressBar__max, 100);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (getBackground() == null && mCircleBackgroundEnabled) {
            ...
            ShapeDrawable BGCircle;
            if (elevationSupported()) {
                BGCircle = new ShapeDrawable(new OvalShape());
                ViewCompat.setElevation(this, SHADOW_ELEVATION * density);
            } else {
                OvalShape oval = new OvalShadow(mShadowRadius, diameter - mShadowRadius * 2);
                BGCircle = new ShapeDrawable(oval);
                ViewCompat.setLayerType(this, ViewCompat.LAYER_TYPE_SOFTWARE, BGCircle.getPaint());
                BGCircle.getPaint().setShadowLayer(mShadowRadius, shadowXOffset, shadowYOffset,
                        KEY_SHADOW_COLOR);
                final int padding = mShadowRadius;
                // set padding so the inner image sits correctly within the shadow.
                setPadding(padding, padding, padding, padding);
            }
            BGCircle.getPaint().setColor(mBackGroundColor);
            setBackgroundDrawable(BGCircle);
        }
        mProgressDrawable.setBackgroundColor(mBackGroundColor);
        mProgressDrawable.setColorSchemeColors(mColors);
        mProgressDrawable.setSizeParameters(diameter, diameter,
                mInnerRadius <= 0 ? 15/*(diameter - mProgressStokeWidth * 2) / 4*/ : mInnerRadius,
                mProgressStokeWidth,
